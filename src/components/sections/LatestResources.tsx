...
const LatestResources = () => {
  ...
  const accentColors = ["var(--gold)", "var(--teal)", "var(--gold)"];
  const accentHex = ["#C9A84C", "#4A9B8E", "#C9A84C"];
        ...
        >
          {displayArticles.map((article, i) => (
            <Link
              ...
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = accentHex[i])}
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden shrink-0">
                ...
                />
                <div className="absolute inset-0" style={{ backgroundColor: "var(--bg-deep)", opacity: 0.35 }} />
                {/* Category tag */}
                <div
                  className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-semibold"
                  style={{ backgroundColor: accentColors[i], color: "var(--text-on-accent)" }}
                >
                  {article.category}
                </div>
              </div>
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <p
                  className="text-sm leading-relaxed line-clamp-3 mb-5 flex-1"
                  style={{ color: "var(--text-muted)" }}
                >
                  {article.excerpt}
                </p>
